...
class PreviewGutter {
  ...
  constructor(editor) {
    ...
    this.markedLines = [];
    this.editor = editor;
    this.id = `${editor.id}${Math.floor(Math.random() * Math.floor(100))}`
    this.markerLayer = editor.addMarkerLayer({
      id: "vertagon_layer_" + this.id
    });
    this.gutter = this.editor.addGutter({
      name: "vertagon-gutter-" + this.id,
      priority: "100",
      type: "decorated"
    });
    this.applyColors();
  ...
  updateGutterSize() {
    if(!this.gutter.element) {
      console.warn("Failed to expand color preview gutter!")
      return;
    }
  ...
  addColorDecorator(color, range, prevDeco = null, isImage = false) {
    ...
    try {
      ...
      let context = this;
      let hoverEvent = pill.addEventListener("mouseenter", e => {
  ...
  clearDecorations() {
    this.markedLines.forEach(line => {
      line.deco.destroy();
      line.colors.forEach((color) => {
        color.element.removeEventListener('mouseenter', color.hoverEvent);
      });
    });
    this.maxPills = 2;
    this.updateGutterSize();
