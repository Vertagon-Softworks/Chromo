...
class PreviewGutter {
  ...
  constructor(editor) {
    this.decorations = [];
    this.editor = editor
    this.markerLayer = editor.addMarkerLayer({ id: "vertagon_layer_" + editor.id });
  ...
  addColorDecorator(color, range, prevDeco = null) {
    ...
    try {
      ...
      console.warn("Chromo failed to create a new marker. This may be because a gutter no longer exists. If the problem persists, please open an issue on the Chromo Github repository")
    }
  }
