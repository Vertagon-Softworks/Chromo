...
export default {
  ...
  convertAnsiToRGB(colorCode, bold, extension){
    return TerminalColors[colorCode]
  },
  ...
  applyColors(){
    ...
    this.editor.scan(/\\u001b\[[0-4]([0-8])((m)|([;]1m)|([;]5[;]([0-2]?[0-9]?[0-9])m))/gi, (obj) =>{
      this.addColorDecorator(this.convertAnsiToRGB(obj.match[1], obj.match[2], obj.match[3]), obj.range);
    });
    ColorNames.forEach((color) => {
